...
# Class for neural network for scren
class NNscreen(torch.nn.Module):
    ...
    def __init__(self, num_frames, b, input_dim, output_dim):
        ...
        self.num_frames = num_frames
        # Convolution 1
        self.conv1 = torch.nn.Conv2d(num_frames, num_frames*b, kernel_size=1)
        # Convolution 2
        self.conv2 = torch.nn.Conv2d(num_frames*b, num_frames*(b**2), kernel_size=1)
        # Linear 1
        self.lin = torch.nn.Linear(input_dim*num_frames*(b**2), output_dim)
    # Forward function
    def forward(self, obs):
